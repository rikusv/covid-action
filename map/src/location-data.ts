...
class LocationData {
  private path = 'https://firestore.googleapis.com/v1/projects/covid-action/databases/(default)/documents/locations'
    ...
    const response: ILocationData[] = await new Promise((resolve) => {
      request.addEventListener('load', () => {
        ...
        .map((fields: any) => ({
          ...
          tags: fields.tags.arrayValue.values.map((value: any) => value.stringValue) || [],
